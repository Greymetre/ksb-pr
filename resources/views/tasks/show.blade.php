         ...
         <div class="col-md-9 main-content1">
            <div class="card">
               ...
               <div class="card-body">
                  ...
                  <div class="row">
                     ...
                     <div class="col-md-4">
                        <div class="input_section">
                           ...
                           <div class="form-group bmd-form-group">
                              <select class="form-control select2" name="task_department_id" id="task_department_id" required style="width: 100%;" {{ $isRestricted?'disabled':''}}>
                        ...
                        <div class="input_section">
                           <label class="col-form-label">{!! trans('panel.task.task_title') !!} <span class="text-danger"> *</span></label>
                              <div class="form-group has-default bmd-form-group" >
                                 <input type="text" name="title" id="title" class="form-control" value="{!! old( 'title', $task['title']) !!}" maxlength="200" required {{ $isRestricted?'readonly':''}}>
                                 @if ($errors->has('title'))
                           ...
                           <div class="input_section">
                              <label class="col-form-label">
                                 <div class="radio-container1">
                                     <label><input type="radio" name="task_type" value="adhoc" {{ $task->task_type=='adhoc'?'checked':''}} {{ $isRestricted?'disabled':''}}> Adhoc</label>
                                     <label><input type="radio" name="task_type" value="customer" {{ $task->task_type=='customer'?'checked':''}} {{ $isRestricted?'disabled':''}}> Customer</label>
                                     <label><input type="radio" name="task_type" value="project" {{ $task->task_type=='project'?'checked':''}} {{ $isRestricted?'disabled':''}}> Project</label>
                                     <label><input type="radio" name="task_type" value="lead" {{ $task->task_type=='lead'?'checked':''}} {{ $isRestricted?'disabled':''}}> Lead</label>
                                 </div>
                              </label>
                              <!-- customers -->
                              <div class="form-group bmd-form-group select-section" id="customer-section">
                                 <select class="form-control select2" name="customer_id" id="customer_id" style="width: 100%;" required {{ $isRestricted?'disabled':''}}>
                              ...
                              <div class="form-group bmd-form-group select-section" id="project-section">
                                 <select class="form-control select2" name="task_project_id" id="task_project_id" required style="width: 100%;" {{ $isRestricted?'disabled':''}}>
                              ...
                              <div class="form-group bmd-form-group select-section" id="lead-section">
                                 <select class="form-control select2" name="lead_id" id="lead_id" style="width: 100%;" required {{ $isRestricted?'disabled':''}}> 
                     ...
                     <div class="col-md-4">
                        <div class="input_section">
                           ...
                           <div class="form-group bmd-form-group">
                              <select class="form-control select2" name="task_priority_id" id="task_priority_id" required style="width: 100%;" {{ $isRestricted?'disabled':''}}>
                     ...
                     <div class="col-md-4">
                        <div class="input_section">
                           <label class="col-form-label">Due Date & Time<span class="text-danger"> *</span></label>
                              <div class="form-group bmd-form-group">
                                 <input type="text" name="due_datetime" id="datetime" class="form-control datetimepicker" value="{!! old( 'due_datetime', $task['due_datetime']) !!}" {{ $isRestricted?'readonly':''}}>
                              </div>
                     ...
                     @if($isRestricted)
                        <div class="col-md-4">
                           <div class="input_section">
                              <label class="col-form-label">Assigned By<span class="text-danger"> *</span></label>
                                 <div class="form-group bmd-form-group">
                                     <select class="form-control select2" name="assigned_by" id="assigned_by" style="width: 100%;"  disabled>
                                         @if(isset($users))
                                             @foreach($users as $user)
                                                 <option value="{{ $user['id'] }}"
                                                    @if(collect(old('assigned_by', $assignedUserIds ?? []))->contains($task['user_id'])) selected @endif>
                                                    {{ $user['name'] }}
                                                </option>
                                 ...
                                 <div class="error">
                                    <p class="text-danger">{{ $errors->first('assigned_by') }}</p>
                                 </div>
                                 @endif
                           </div>
                        </div> 
                        ...
                        <div class="col-md-4">
                           <div class="input_section">
                              <label class="col-form-label">Status<span class="text-danger"> *</span></label>
                                 <div class="form-group bmd-form-group">
                                     <select class="form-control select2" name="task_status" id="task_status" style="width: 100%;" {{ ($task['task_status']=='Completed') && !(auth()->user()->hasRole('superadmin')) ?'disabled':''}} >
                                         @if(isset($taskStatuses))
                                             @foreach($taskStatuses as $taskStatus)
                                                 <option value="{{ $taskStatus }}"
                                                    @if($taskStatus == $task['task_status']) selected @endif>
                                                    {{ $taskStatus }}
                                                </option>
                                 ...
                                 <div class="error">
                                    <p class="text-danger">{{ $errors->first('task_status') }}</p>
                                 </div>
                                 @endif
                           </div>
                        </div> 
                  ...
                  <div class="row">
                     <div class="col-md-12 ">
                        <div class="form-group has-default bmd1-form-group">
                           <textarea class="ckeditor1 form-control" rows="8" cols="30" name="descriptions" id="descriptions" {{ $isRestricted?'readonly':''}} required>{!! old( 'descriptions', $task['descriptions']) !!}</textarea>
                  ...
                  <!-- comments -->
                     <hr class="my-3">
                     <h4 class="section-heading mb-3  h4 mt-0 text-center">Comments</h4>
                     <div class="row">
                        @if(count($taskComments))
                           @foreach($taskComments as $taskComment)
                              <div class="col-md-12">
                                 <div class="form-group has-default bmd1-form-group">
                                    <label>{{ optional($taskComment->user)->name }} &nbsp;&nbsp; {{ date('d-m-Y h:i a',strtotime($taskComment->created_at)) }} </label>
                                    <textarea class="ckeditor1 form-control" rows="4" cols="30" readonly >{!! old( 'comment', $taskComment) !!}</textarea>
                     ...
                     @if($task['task_status']!='Completed' ||  auth()->user()->hasRole('superadmin'))
                        <div class="row">
                           <div class="col-md-12">
                              <div class="form-group has-default bmd1-form-group">
                                 <textarea class="ckeditor1 form-control" rows="8" cols="30" name="comment" id="comment" required>{!! old( 'comment', '') !!}</textarea>
         ...
         <div class="col-3 sidebar1">
            ...
            <div class="card blck-clr">
                 <div class="card-body">
                     <div class="row">
                         <div class="col-12">
                             <h3 class="card-title pb-3">Task Activity Logs</h3>
                             ...
                             <!-- Log Entry -->
                              @if(count($taskStatusLogs))
                                 @foreach($taskStatusLogs as $taskStatusLog)
                                   <div class="d-flex mb-2 siderbar-content">
                                       <i class="material-icons">double_arrow</i>
                                       <p class="mb-0 activity-msg">
                                          {{ $taskStatusLog->comments??null }}
                                       </p>
                                   </div>
                                 @endforeach  
                              @endif  
