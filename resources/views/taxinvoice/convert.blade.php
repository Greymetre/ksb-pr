    ...
    <div class="container-fluid">
      <div class="card p-4">
        <form action="{{ route('tax_invoice.store') }}" method="post" id="createInvoiceForm" enctype="multipart/form-data">
          @csrf
          <h5><i class="fa fa-file-text-o"></i> Convert To Invoice</h5>
          <div class="row">
            <div class="col-md-8">
                ...
                <div class="input-group">
                  <div class="innerborder">
                    ...
                    <select class="custom-select select2" id="customer_id" name="customer_id" required>
                      ...
                      <option value="{!! $customer['id'] !!}" {{ old('customer_id', $convert_estimate->customer_id) == $customer['id'] ? 'selected' : '' }}>{!! $customer['name'] !!}</option>
                      @endforeach
                      @endif
                    </select>
                  </div>
          ...
          <div class="row">
            ...
            <div class="col-md-4">
              <div class="form-group mb-3">
                ...
                <input type="text" class="form-control custom-input-box" id="order_no" name="order_no" value="{!! old('order_no', $convert_estimate->order_no) !!}">
              </div>
            </div>
          </div>
          <div class="row">
            <div class="col-md-3">
              <div class="form-group mb-3">
                ...
                <input type="date" class="form-control custom-input-box" id="invoice_date" name="invoice_date" value="{!! old('invoice_date', date('Y-m-d', strtotime($convert_estimate->estimate_date))) !!}" required>
              </div>
            </div>
            <div class="col-md-3">
              <div class="form-group mb-3 bmd-form-group">
                ...
                <select class="form-control select2 custom-select-box" id="user_id" name="user_id">
                  ...
                  <option value="{!! $user['id'] !!}" {{ old( 'user_id', $convert_estimate->user_id) == $user['id'] ? 'selected' : '' }}>{!! $user['name'] !!}</option>
                  @endforeach
                  @endif
                </select>
              </div>
            ...
            <div class="col-md-3">
              <div class="form-group mb-3 bmd-form-group">
                ...
                <select class="form-control select2 custom-select-box" id="payment_term" name="payment_term">
                  ...
                  <option value="{{ $term->id }}" data-days="{{ $term->number_of_days }}" {{ old( 'payment_term', $convert_estimate->payment_term) == $term->id ? 'selected' : '' }}>{{ $term->term_name }}</option>
                  @endforeach
                  @endif
                  <option value="__add__">➕ Add New</option>
                </select>
            ...
            <div class="col-md-3">
              <div class="form-group mb-3">
                ...
                <input type="date" class="form-control custom-input-box" value="{!! old('due_date', date('Y-m-d', strtotime($convert_estimate->due_date))) !!}"  id="due_date" name="due_date">
              </div>
            </div>
          </div>
          <div class="row">
              ...
              <div class="table-responsive">
                <table class="table table-bordered table-sm">
                  ...
                  <tbody id="item-table-body">
                    @if(!empty($convert_estimate->details))
                    @foreach($convert_estimate->details as $item)
                    <tr>
                      <td>
                        <select class="form-control select2 product_rowchange" id="product_id" name="product_id[]" required>
                          ...
                          <option value="{!! $product['id'] !!}" {{ old( 'product_id', $item->product_id) == $product['id'] ? 'selected' : '' }}>{!! $product['display_name'] !!}</option>
                          @endforeach
                          @endif
                        </select>
                        <input type="text" class="form-control d-none" placeholder="Description" name="product_dec[]" value="{!! old('product_dec', $item->product_dec) !!}" id="product_dec" />
                      </td>
                      <td><input type="text" name="hsn_sac[]" value="{!! old('hsn_sac', $item->hsn_sac) !!}" /></td>
                      <td><input type="number" class="form-control quantity_rowchange" name="quantity[]" min="0" step="0.01" value="{{ old('quantity', $item->quantity)}}" /></td>
                      <td><input type="number" class="form-control mrp_rowchange" name="mrp[]" min="0" step="0.01" value="{{ old('mrp', $item->mrp)}}" /> <input type="hidden" name="tax_amount[]" value="{{ old('tax_amount', $item->tax_amount)}}"></td>
                      <td>
                        <select name="tax[]" class="form-control tax_rowchange">
                          ...
                          <option value="{!! $tax['id'] !!}" data-rate="{!! $tax['tax_percentage'] !!}" {{ old( 'tax', $item->tax) == $tax['id'] ? 'selected' : '' }}>{!! $tax['tax_name'] !!} ({!! $tax['tax_percentage'] !!}% )</option>
                          @endforeach
                          @endif
                        </select>
                      </td>
                      <td><input type="text" class="amount" name="amount[]" value="{{ old('amount', $item->amount)}}" readonly /></td>
            ...
            <div class="col-md-4">
              <div class="form-group">
                ...
                <textarea
                  ...
                  rows="4"
                  name="customer_notes"
                  placeholder="Note...">{{ old('customer_notes', $convert_estimate->customer_notes) }}</textarea>
              </div>
            </div>
            <div class="col-md-4">
                ...
                <div class="form-group d-flex align-items-center">
                  ...
                  <div class="col-6 px-1">
                    <select class="form-control form-control-sm" name="tds" id="tds">
                      ...
                      <option value="{!! $tds['id'] !!}" data-rate="{!! $tds['rate'] !!}" {{ old( 'tds', $convert_estimate->tds) == $tds['id'] ? 'selected' : '' }} >{!! $tds['tax_name'] !!} ({!! $tds['rate'] !!}% )</option>
                      @endforeach
                      @endif
                    </select>
                  </div>
                ...
                <div class="form-group d-flex align-items-center">
                  ...
                  <div class="col-6 px-1">
                    <input
                      ...
                      step="0.01"
                      value="{{ old('adjustment', $convert_estimate->adjustment) }}"
                      id="adjustment"
                      name="adjustment"
                      placeholder="Text" />
                  </div>
            ...
            <div class="col-md-12">
              <div class="form-group">
                ...
                <textarea
                  ...
                  rows="4"
                  name="t_c"
                  placeholder="Terms & Conditions ....">{{ old('t_c', $convert_estimate->t_c) }}</textarea>
              </div>
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="button-section">
                <input type="hidden" name="convert_estimate_id" value="{{ $convert_estimate->id }}">
