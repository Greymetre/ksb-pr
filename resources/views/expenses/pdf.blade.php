...
$grandTotal = 0;
$expenseTypeWidth = $expenseTypes->count() ? min(13, max(6, 34 / $expenseTypes->count())) : 10;
@endphp
<div class="report">
...
<th style="width:7%">Travel Date</th><th style="width:14%">From Place</th><th style="width:14%">To Place</th><th style="width:6%">Starting Time</th><th style="width:6%">End Time</th><th style="width:5%">Night Halt</th>
...
$typeTotals[$type['name']] += $amount; $dayTotal += $amount;
@endphp
...
@endforeach
<tr class="grand"><td colspan="6" class="center">GRAND TOTAL</td>@foreach($expenseTypes as $type)<td class="right">{{ $typeTotals[$type['name']] > 0 ? number_format($typeTotals[$type['name']], 2) : '-' }}</td>@endforeach<td class="right">{{ number_format($grandTotal, 2) }}</td><td></td></tr>
</tbody></table>
