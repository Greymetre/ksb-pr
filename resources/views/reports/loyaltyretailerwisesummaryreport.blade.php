<x-app-layout>
  <section class="fk-manual-listing">
    <div class="fk-list-page-head">
      <div class="fk-list-heading-block">
        <div class="fk-list-breadcrumb">
          <span>REPORTS</span>
          <span>›</span>
          <span class="fk-current">LOYALTY SUMMARY RETAILER WISE REPORT</span>
        </div>
        <div class="fk-list-title-row">
          <h1 class="fk-list-title">Loyalty Summary Retailer Wise Report</h1>
          <span id="loyaltyRetailerCount" class="fk-list-count is-visible">0 records</span>
        </div>
      </div>
      <div class="fk-list-actions">
        <button type="button" class="btn fk-filter-trigger" data-filter-target="#loyaltyRetailerFilters">
    ...
    <aside id="loyaltyRetailerFilters" class="fk-filter-drawer" aria-hidden="true">
      ...
      <div class="fk-filter-drawer-body">
        @if(auth()->user()->can(['customer_download']))
          <form method="GET" action="{{ URL::to('loyalty-retailer-wise-report-download') }}" id="loyaltyRetailerExportForm">
            <input type="hidden" name="pageSize" id="export_pageSize">
      ...
      <div class="fk-filter-drawer-tools">
        @if(auth()->user()->can(['customer_download']) && auth()->user()->can('loyalty_summary_dealer_wise_report_download'))
          <button type="button" id="loyaltyRetailerExportBtn" class="btn fk-tool-export">
      ...
      <div class="card-body">
        ...
        <div class="fk-table-meta">
          ...
          <div class="fk-table-meta-copy">
            <h2>Loyalty Summary Retailer Wise Report</h2>
            <p id="loyaltyRetailerMeta" class="fk-table-meta-subline">Live directory · page 1 of 1</p>
          </div>
        </div>
  ...
  <script type="text/javascript">
    $(function() {
      ...
      function updateLoyaltyRetailerMeta(table) {
        var info = table.page.info();
        var total = info.recordsDisplay || info.recordsTotal || 0;
        var pages = info.pages || 1;
        $('#loyaltyRetailerCount').text(total + ' records').toggleClass('is-visible', true);
        $('#loyaltyRetailerMeta').text('Live directory · page ' + ((info.page || 0) + 1) + ' of ' + pages);
      }
      ...
      var table = $('#getbranchsummarydealerwise').DataTable({
        ...
        order: [[0, 'desc']],
        ajax: {
          url: "{{ route('loyaltyRetailerWiseSummaryReport') }}",
          data: function(d) {
        ...
        columns: [
          ...
          { data: 'customeraddress.cityname.city_name', name: 'customeraddress.cityname.city_name', defaultContent: '', orderable: false, searchable: false },
        ...
        drawCallback: function () {
          updateLoyaltyRetailerMeta(this.api());
        }
      });
      ...
      $('#loyaltyRetailerExportBtn').click(function(e) {
        e.preventDefault();

        var pageInfo = table.page.info();
        $('#export_pageSize').val(pageInfo.length);
        $('#export_page').val(pageInfo.page + 1);
        $('#loyaltyRetailerExportForm').submit();
      });

      table.on('xhr.dt', function () {
        setTimeout(function () {
          updateLoyaltyRetailerMeta(table);
        }, 0);
      });
    });
  </script>
