...
class GiftRedemptionDataTable extends DataTable
{
        ...
        return datatables()
            ...
            ->editColumn('status', function ($data) {
                if ($data->status == '0') {
                    ...
                    return '<button id="' . $data->id . '" data-status="1" class="ChangeStatusGift btn btn-primary">Approve</button>';
                } elseif ($data->status == '3') {
                    return '<button id="' . $data->id . '" data-status="3" class="deliveredStatus btn btn-primary">Dispatch</button>';
    ...
    public function query(Redemption $model, Request $request)
    {
        ...
        $data = $model->with('customer', 'product')->where('redeem_mode', '1');
