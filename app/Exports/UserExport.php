...
class UserExport implements FromCollection, WithHeadings, ShouldAutoSize, WithMapping
{
    ...
    {
        if (Auth::user()->id == 1) {
            return ['ID', 'Employees Code', 'User Name', 'Designation', 'Role', 'Branch Name', 'Base Location', 'Department', 'Zone', 'Reporting To', 'Mobile', 'Email',
            //  'Gender',
        ...
        if (Auth::user()->id == 1) {
            return [
               ...
               $data['personal_number'] ?? '-',
                ...
                $data['department_id'] ?? '-',
                // $data['warehouse_id'] ?? '-',
                ($data['show_attandance_report'] == 1 ? '1' : '0'),
                $data['sales_type'] ?? '',
